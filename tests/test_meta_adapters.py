...
def test_facebook_messenger_inbound_and_outbound(monkeypatch, tmp_path: Path) -> None:
    connector = {"id": "facebook-one", "direction": "bidirectional", "page_id": "page-1",
    ...
    monkeypatch.setattr("mailbox_channels.adapters.facebook_messenger_adapter.connectors_for", lambda adapter: [connector])
    session = Session()
    ...
    adapter.send_message({"connector_id": "facebook-one", "channel_id": "user-1", "text": "reply"})
...
def test_whatsapp_business_inbound_outbound_and_media(monkeypatch, tmp_path: Path) -> None:
    connector = {"id": "whatsapp-one", "direction": "bidirectional", "phone_number_id": "phone-1",
    ...
    monkeypatch.setattr("mailbox_channels.adapters.whatsapp_adapter.connectors_for", lambda adapter: [connector])
    session = Session()
    ...
    adapter.send_message({"connector_id": "whatsapp-one", "channel_id": "15551234567",
...
def test_whatsapp_business_group_preserves_group_and_participant(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("WHATSAPP_VERIFY_TOKEN", "verify")
    monkeypatch.setenv("WHATSAPP_APP_SECRET", "app-secret")
    monkeypatch.setattr("mailbox_channels.adapters.whatsapp_adapter.connectors_for",
    ...
    adapter.send_message({"connector_id": "whatsapp-one", "channel_id": "group-1",
...
def test_inbound_meta_adapters_require_webhook_credentials(monkeypatch) -> None:
    ...
    monkeypatch.setattr("mailbox_channels.adapters.facebook_messenger_adapter.connectors_for",
