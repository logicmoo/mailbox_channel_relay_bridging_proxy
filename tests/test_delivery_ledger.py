...
def test_origin_survives_relay_message_ids() -> None:
    fields = with_origin({}, adapter="irc", connector_id="libera", source_id="msg-1", channel_id="#agents")
...
def test_delivery_claim_is_durable_and_atomic(tmp_path: Path) -> None:
    ...
    endpoint = endpoint_id("irc", connector_id="libera", channel_id="#agents")
...
def test_same_origin_can_traverse_distinct_endpoints(tmp_path: Path) -> None:
    ledger = DeliveryLedger(tmp_path)
    message = {"id": "mail-1", "origin_id": "origin:one"}
    assert ledger.claim(message, endpoint_id("irc", connector_id="one", channel_id="#a"))
    assert ledger.claim(message, endpoint_id("irc", connector_id="two", channel_id="#a"))
